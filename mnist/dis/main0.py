    ...
    for iiter in range(args.global_iteration1): 
        ...
        for iter in range(args.global_iteration2): 
            ...
            for k in range(args.num_clients):
                ...
                _,task_model[k],FC1[k],FC2[k],stop[k] = solver[k].train(querry_dataloader[k],val_dataloader[k],task_model[k],FC1[k],FC2[k],unlabeled_dataloader[k],lr,1,iter)
        ...
        ###################################################################################
        for k in range(args.num_clients):
            DL_dis+=solver[k].DLcal(R_task_model[k],R_FC1[k], R_FC2[k],querry_dataloader[k])
            Num_dis+=len(labeled_indices[k])
            DL_item.append(solver[k].DLcal(R_task_model[k],R_FC1[k], R_FC2[k],querry_dataloader[k])/len(labeled_indices[k])/args.num_classes)
        
        DL_dis=DL_dis/Num_dis/args.num_classes
                            
        if iiter!=args.global_iteration1-1:
            for k in range(args.num_clients):
                sampled_indices = solver[k].sample_for_labeling(R_task_model[k],R_FC1[k], R_FC2[k], DL_dis ,DL_item[k] ,unlabeled_dataloader[k])
