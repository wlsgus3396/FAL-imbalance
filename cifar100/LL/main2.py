    ...
    for iiter in range(args.global_iteration1): 
        ...
        for iter in range(args.global_iteration2): 
            ...
            for k in range(args.num_clients):     
                ...
                _, task_model[k],_ ,stop[k] = solver[k].train(querry_dataloader[k],val_dataloader[k],task_model[k],LN[k],unlabeled_dataloader[k],lr,0,iter)
        ...
        for iter in range(1): 
            ...
            for k in range(args.num_clients):  
                ...
                Soloaccuracy, task_model[k],LN[k] = solver[k].train(querry_dataloader[k],val_dataloader[k],task_model[k],LN[k],unlabeled_dataloader[k],lr,1,iter)
        ...
        if iiter!=args.global_iteration1-1:
            for k in range(args.num_clients):
                sampled_indices = solver[k].sample_for_labeling(task_model[k],LN[k] ,unlabeled_dataloader[k])
